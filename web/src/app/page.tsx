...
for (const f of dining) {
  for (const area of kantoAreas) {
    if (area.cities.length > 0) {
      if (area.prefs.includes(f.prefecture || "") && area.cities.includes(f.city || "")) area.count++;
    } else {
      if (area.prefs.includes(f.prefecture || "")) area.count++;
    }
  }
}
...
const eras = [
  { name: "〜江戸", range: "〜1603", min: 0, max: 1603 },
...
const TEAL = "linear-gradient(170deg, #1a5c6e 0%, #144a59 40%, #0d3a47 70%, #0a2e38 100%)";
const TEAL_MID = "linear-gradient(135deg, #144a59 0%, #0d3a47 100%)";
const GREEN = "linear-gradient(135deg, #1d764a 0%, #144a30 100%)";

export default function Home() {
  return (
    <div>
      {/* ━━━ HERO ━━━ full screen, deep teal */}
      <section className="relative overflow-hidden min-h-screen flex flex-col justify-end" style={{ background: TEAL }}>
        <div className="absolute inset-0 overflow-hidden">
          {seedColors.slice(0, 6).map((color, i) => (
            <div key={i} className="absolute rounded-full seed-float"
              style={{
                backgroundColor: color, width: `${50 + i * 14}px`, height: `${70 + i * 18}px`,
                top: `${10 + (i * 37) % 80}%`, left: `${5 + (i * 43) % 90}%`, opacity: 0.06,
        ...
        <div className="relative z-10 max-w-[1400px] mx-auto px-6 lg:px-12 pb-24 md:pb-32 w-full">
          ...
          <div className="flex flex-wrap gap-4">
            <Link href="/facilities" className="inline-block bg-white text-[var(--color-accent)] px-10 py-4 text-sm tracking-[0.2em] hover:bg-white/90 transition-colors">食を探す</Link>
            <Link href="/stays" className="inline-block border-[1.5px] border-white/30 text-white px-10 py-4 text-sm tracking-[0.2em] hover:border-white/60 hover:bg-white/5 transition-all">宿を探す</Link>
          </div>
        </div>
      ...
      <div className="bg-[var(--color-green)] text-white overflow-hidden whitespace-nowrap py-4">
        <div className="inline-flex" style={{ animation: "marqueeScroll 25s linear infinite" }}>
          {["創業100年を超える老舗 87店舗", "登録有形文化財の建物で味わう至福", "江戸から令和まで、食の場所性を辿る", "トレンドではなく、歴史と文化で選ぶ"]
            .flatMap((t, i) => [
              <span key={`a${i}`} className="font-serif text-[1.1rem] tracking-wider px-10">{t}</span>,
              <span key={`b${i}`} className="inline-block w-1.5 h-1.5 rounded-full bg-white/25 mx-2 align-middle" />,
            ]).concat(
              ["創業100年を超える老舗 87店舗", "登録有形文化財の建物で味わう至福", "江戸から令和まで、食の場所性を辿る", "トレンドではなく、歴史と文化で選ぶ"]
                .flatMap((t, i) => [
                  <span key={`c${i}`} className="font-serif text-[1.1rem] tracking-wider px-10">{t}</span>,
      ...
      {/* ━━━ STATS ━━━ teal bg, white text — extending hero tone */}
      <section className="py-20" style={{ background: TEAL_MID }}>
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                <Link key={i} href="/facilities"
                  className="bg-white/6 border border-white/10 p-6 text-sm text-white/70 leading-relaxed hover:bg-white/15 hover:border-white/25 transition-all group">
                  <span className="font-serif text-2xl text-white/15 group-hover:text-white/30 block mb-3 transition-colors">{opt.glyph}</span>
          ...
          <div className="mt-8 flex flex-wrap gap-2">
            {[
              ...
              <Link key={r.name} href={r.href}
                className="text-sm px-6 py-3 border-[1.5px] border-[var(--color-accent)] text-[var(--color-accent)] hover:bg-[var(--color-accent)] hover:text-white transition-all">
                {r.name}
              </Link>
            ))}
          </div>
      ...
      <section className="py-24">
        <div className="max-w-[1400px] mx-auto px-6 lg:px-12">
          ...
          <div className="relative">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-7 gap-px bg-[var(--color-border)]">
              {eras.map((era, i) => (
                <Link key={i} href={`/facilities?era=${era.min}-${era.max}`}
                  className="group relative z-10 bg-[var(--color-bg)] p-5 text-center hover:bg-[var(--color-accent)] transition-colors">
                  <p className="font-serif text-[2rem] text-[var(--color-accent)]/10 group-hover:text-white/15 leading-none mb-2 transition-colors">
        ...
        <div className="max-w-[1400px] mx-auto px-6 lg:px-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <a href="/report-bashosei.html" className="group block overflow-hidden bg-white/4 border border-white/8 hover:bg-white/8 transition-colors">
              <div className="aspect-[2/1] flex items-center justify-center text-center p-8" style={{ background: GREEN }}>
                <div>
                  <p className="text-[0.6rem] tracking-[0.4em] text-white/30 mb-3">REPORT</p>
                  <p className="font-serif text-xl md:text-2xl font-bold text-white leading-relaxed">場所性（Basho-sei）<br />とは何か</p>
                </div>
              </div>
              <div className="p-6">
            ...
            <Link href="/about" className="group block overflow-hidden bg-white/4 border border-white/8 hover:bg-white/8 transition-colors">
              <div className="aspect-[2/1] flex items-center justify-center text-center p-8"
                style={{ background: "linear-gradient(135deg, #1a5c6e, #0a2e38)" }}>
                <div>
                  <p className="text-[0.6rem] tracking-[0.4em] text-white/30 mb-3">ABOUT</p>
                  <p className="font-serif text-xl md:text-2xl font-bold text-white leading-relaxed">場所性の<br />8軸フレームワーク</p>
                </div>
              </div>
              <div className="p-6">
                <p className="text-[0.82rem] text-white/50 leading-relaxed">
        ...
        <div className="max-w-[1400px] mx-auto px-6 lg:px-12">
          ...
          <div className="flex gap-3">
            <input type="text"
              ...
              className="flex-1 px-6 py-5 bg-white border border-[var(--color-border)] text-sm placeholder:text-[var(--color-text-light)]/40 focus:outline-none focus:border-[var(--color-accent)] transition-colors"
              disabled />
            <button className="px-10 py-5 bg-[var(--color-accent)] text-white text-sm tracking-wider opacity-40 cursor-not-allowed" disabled>検索</button>
          </div>
          <p className="text-xs text-[var(--color-text-light)]/30 mt-4">Coming Soon — AI検索機能は現在開発中です</p>
        </div>
      </section>
        ...
        <div className="max-w-[1400px] mx-auto px-6 lg:px-12">
          ...
          <div className="grid grid-cols-3 gap-3">
            {sortedCategories.map(([cat, count], idx) => {
              const bgs = [TEAL_MID, GREEN, "linear-gradient(135deg, #B07256, #7A4033)"];
              return (
                <Link key={cat} href={`/facilities?category=${cat}`}
                  className="group relative aspect-[3/2] overflow-hidden flex items-end">
                  <div className="absolute inset-0 group-hover:scale-[1.06] transition-transform duration-700" style={{ background: bgs[idx % 3] }} />
                  <div className="absolute inset-0" style={{ background: "linear-gradient(0deg, rgba(0,0,0,0.5) 0%, transparent 60%)" }} />
                  <span className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 font-serif text-[10rem] font-bold text-white/[0.03] group-hover:text-white/[0.07] transition-colors">
      ...
      {/* ━━━ CREDIT ━━━ */}
      <section className="bg-[var(--color-bg)] py-16 text-center">
        <p className="text-[0.6rem] tracking-[0.4em] text-[var(--color-text-light)] mb-3">CREATED BY</p>
