      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-10">
        {sorted.map((f) => (
          <Link key={f.id} href={`/facilities/${f.id}`} className="group block">
            {/* Image placeholder */}
            <div className="aspect-[4/3] bg-stone-200 mb-4 overflow-hidden relative">
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <span className="font-serif text-5xl text-stone-300/40 group-hover:scale-110 transition-transform">
                  {f.name.charAt(0)}
                </span>
              </div>
              {/* Category badge */}
              <div className="absolute top-3 left-3">
                <span className="text-xs px-2 py-1 bg-stone-900/70 text-white rounded">
